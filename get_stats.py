...
def get_hitting(start_season = 1876, end_season = current_year):
    ...
    hitting_df.to_csv(current_directory + '\\data\\hitting_data.csv')
...
def get_pitching(start_season = 1876, end_season = current_year):
    ...
    for y in years:
        ...
        print(f'Finished {y} season.')
    # Data saved as csv in current working directory
    pitching_df.to_csv(current_directory + '\\data\\pitching_data.csv')
...
def get_war(start_season = 1871, end_season = current_year):
    ...
    for y in years:
        if break_program:
            break
        api_url = f'https://www.fangraphs.com/api/leaders/war?season={y}&lg=&wartype=0&teamid='
        make_request = requests.get(url = api_url).json()
        temp_df = pd.DataFrame(make_request)
        war_df = pd.concat([war_df, temp_df], axis = 0)
        print(f'Finished {y} season.')
    # Data saved as csv in current working directory
    war_df.to_csv(current_directory + '\\data\\war_data.csv')
...
def get_contracts():
    ...
    for t in t_id:
        ...
        print(f'Finished team {t}.')
    # Data saved as csv in current working directory
    contracts_df.to_csv(current_directory + '\\data\\contract_data.csv')
